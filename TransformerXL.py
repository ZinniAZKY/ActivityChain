...
def load_datasets(tokenizer, train_file, val_file, enhance_file):
    ...
    eval_dataset = TextDataset(val_file, tokenizer)
...
def initialize_model_and_training_args(tokenizer, args):
    config = TransfoXLConfig.from_pretrained(
        ...
        n_head=args.n_head,
        d_model=args.n_emb,  # Note that d_model is used instead of n_embd for Transformer XL
    )
    ...
    training_args = TrainingArguments(
        ...
        save_steps=3,
        report_to=["wandb"],
        # the gap of printing loss between epochs
        logging_steps=250,
        do_eval=True,
        eval_steps=1,
